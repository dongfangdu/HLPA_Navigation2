...
def hotword_mining():
    ...
    text = st.text_area("🍄 请输入体验文本:", value = default_sample, height = 200, key="public_ner_sample_parser")
    ...
    if st.button("点击解析"):
        ...
        try:
            parser_res = requests.post(url = URL + "parse", data = json.dumps({"q": json.dumps(text)})).json().get("Content", [])
            st.success("解析完成")
            st.dataframe(pd.DataFrame.from_dict(parser_res))
    ...
    if st.checkbox("点击查看 接口文档"):
        ...
        if option == "入参 JSON":
            ...
            st.table(pd.DataFrame({
                "属性": ["q"],
                "类型": ["Optional[List[String], String]"],
                "说明": ["待解析文本列表"]
            }))
