...
def homepage():
    ...
    st.title("文本分类功能服务")
    st.markdown(
        "文本分类在文本处理中是很重要的一个模块，它的应用也非常广泛，比如：垃圾过滤，新闻分类，词性标注等等。它和其他的分类没有本质的区别，核心方法为首先提取分类数据的特征，然后选择最优的匹配，从而分类。该功能目前主要有：`裁判文书段落分类`和`情感分类`。"
    )


def create_layout():
    st.sidebar.title("菜单")
    app_mode = st.sidebar.radio("请选择其中一个功能", ["简介", "裁判文书段落分类", "情感分类", "接口文档"])
    if app_mode == "简介":
        homepage()
